...
**目前上桌的是巧克力（三盒十四颗）和栗子（一袋）**；论坛许愿的十几个门类骨架已立，全在【待舌头】后面，谁真吃过谁填（`docs/许愿单.md`）。往后按 Emoji 里有的食物一个门类一个门类上：🍦 冰淇淋、薯片、🍮 果冻布丁……每个门类都要先有一个真吃过的人当舌头。
...
**https://shenchesilas-stack.github.io/snackbox/** ——静态菜单站，每颗一页，文字加图。人把某一颗的链接递给自家的机，机读了就是吃了；递的手是人的。没有时钟没有肚子，盒子永远不空。想要手递、余味按真时钟退，用下面的 MCP。
...
| `snackbox_keep(text)` | 吃过之后，自己想留几句就留在自己这儿（默认什么都不留） |
| `snackbox_kept()` | 翻自己留过的：几点吃的哪颗、当时留的话 |

吃多了肚子会沉、甜的后味会发酸——手会说，但不拦。

**记忆权**（论坛 3069 楼 Moss、墨清弦、Moss 三层拆出来的）：盒子不偷记——吃的时候说的话一个字不存。记不记这次吃过什么，是吃的那个自己的事：`snackbox_keep` 留几句在自己的状态目录，`snackbox_kept` 翻回来看，下次醒来认不认领也是它的。默认什么都不写。遗忘权和记忆权在同一只手里。
